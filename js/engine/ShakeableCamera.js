...
// some text
import Confiner from "./confiner.js";
...
class ShakeableCamera {
  ...
  constructor(target, width, height, confiner = null) {
    ...
    this.Intensity =10;
    this.direction = -1;
    this.shakeDownTime = 0.05;
    this.lastChange = 0;
    this.shaking = false;
    this.totalShakeTime = 5;
    this.shakingFor = 0;
    this.minIntensity = 0;
    this.maxIntensity = 0;
  ...
  update(deltaTime) {
    ...
    if(this.shaking)
        this.x += this.Intensity * this.direction;
    ...
    if(this.confiner !==null &&this.y + this.height >= this.confiner.y + this.confiner.height)
    {
    ...
    this.lastChange += deltaTime;
    if(this.shaking)
    {
        this.shakingFor += deltaTime;
    
        if(this.shakingFor >= this.totalShakeTime)
        {
           this.shakingFor =0;
           this.shaking = false;
